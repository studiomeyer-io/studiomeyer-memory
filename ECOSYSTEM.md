# StudioMeyer Ecosystem

StudioMeyer Memory works best with the rest of the suite. All products share the same account via Magic Link.

## MCP Server Products

| Product | Tools | Connect | Pricing |
|---------|-------|---------|---------|
| **[Memory](https://github.com/studiomeyer-io/studiomeyer-memory)** | 56 | `memory.studiomeyer.io/mcp` | Free / 9€ / 19€ |
| **[CRM](https://github.com/studiomeyer-io/studiomeyer-crm)** | 37 | `crm.studiomeyer.io/mcp` | Free / 9€ / 19€ |
| **[GEO](https://github.com/studiomeyer-io/studiomeyer-geo)** | 30 | `geo.studiomeyer.io/mcp` | Free |
| **[Crew](https://github.com/studiomeyer-io/mcp-crew)** | 16 | `crew.studiomeyer.io/mcp` | Free |

## Claude Code Plugin Suite

Install everything with one command:
```bash
/plugin marketplace add studiomeyer-io/studiomeyer-marketplace
```

119 tools, 21 slash commands, 5 skills, 3 subagents. [Marketplace repo](https://github.com/studiomeyer-io/studiomeyer-marketplace).

## Open Source Tools

| Project | What it does |
|---------|-------------|
| **[AI Shield](https://github.com/studiomeyer-io/ai-shield)** | LLM security middleware (325 tests, zero deps) |
| **[Darwin Agents](https://github.com/studiomeyer-io/darwin-agents)** | Self-evolving AI agents with A/B testing |
| **[Agent Fleet](https://github.com/studiomeyer-io/agent-fleet)** | Multi-agent orchestration for Claude Code |
| **[MCP Video](https://github.com/studiomeyer-io/mcp-video)** | Video production via MCP (FFmpeg + Playwright) |

## How Memory connects

Memory is the persistence layer. CRM stores contact data, GEO tracks AI visibility, Crew provides personas — and Memory remembers everything across sessions. Connect all four for a complete AI workspace.

---

Built by [StudioMeyer](https://studiomeyer.io) — AI agency from Mallorca, Spain.
